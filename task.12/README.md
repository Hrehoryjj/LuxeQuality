...
| TC-03 | Verify All Products and product detail page | Positive | Cypress `cy.prompt` | <!-- FILL: `npx cypress run` could not be executed in the authoring session (see Limitations); confirm locally/in CI --> |
| TC-04 | Search Product | Positive | Cypress `cy.prompt` | <!-- FILL: same as TC-03 --> |
| TC-05 | Verify Subscription in home page | Positive | Cursor | ✅ Passing |
| TC-06 | Remove Products From Cart | Positive | Claude Code, Cursor, Cypress `cy.prompt` | ✅ Passing (Claude Code, Cursor). Cypress: <!-- FILL: same as TC-03 --> |
| TC-07 | Login with incorrect email or password | Negative | Claude Code | ✅ Passing |
...
- **Vague AI oracle.** TC-03's original assertions checked that the detail page *has* a
  ...
  replaced with deterministic assertions in Phase 2 (see Hybrid-oracle approach above).
